...
with open("/etc/kafetch/ascii", encoding='unicode_escape') as file:
  ascii['content'] = file.read()
  file.seek(0)
  for i in file.read().splitlines():
    if len(escape_ansi(i)) + 1 > ascii['rown']:
      ascii['rown'] = len(escape_ansi(i)) + 1
  file.seek(0)
  ascii['coln'] = len(file.read().splitlines())
...
print(f"\033[{ascii['coln']}A", end="")
print(f"\033[{ascii['rown']}C", end="")
